...
impl Cutscene for IntroCutscene {
    fn build(cutscene: &mut CutsceneBuilder) {
        cutscene.add_timed_step(step1, 1.1); // 14
...
fn reset_fast(mut screen_fade: ResMut<ScreenFade>, state: Res<IntroCutsceneState>) {
    if !state.proceed {
